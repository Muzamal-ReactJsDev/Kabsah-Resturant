...
//           {!isLoading && !isLoading2 ? (
//             <>
//               <h5 className="sandwich">{data?.GRILLED.name}</h5>
//               {data?.GRILLED.products.map((dataGrilled, index) => (
//                 <Col
//                   xs={6}
//                   className="for-sandwich"
//                   key={index}
//                   onClick={() => handleShowModal(dataGrilled)}
//                 >
//                   <Row className="sandwich-cards">
//                     <Col xs={3} className="sandwich-image-col">
//                       <img
//                         className="sandwich-image"
//                         src={`${data2?.base_urls?.product_image_url}/${dataGrilled?.image}`}
//                         // src={dataGrilled.image}
//                         alt=""
//                       />
//                     </Col>
//                     <Col xs={7} className="sandwich-names-price">
//                       <div className="sandwich-both-name-discount">
//                         <h7>{dataGrilled.name}</h7>
//                         <br />
//                         <h7>${dataGrilled.price}</h7>
//                       </div>
//                     </Col>
...
const Index = () => {
  const { data, isLoading, isError, isFetching, isSuccess } =
    useGetAllPostQuery();
  // console.log("data for the Grilled", data[4]);
  console.log("data for the Grilled", data);
  const {
    data: data2,
    isLoading: isLoading2,
    isError: isError2,
    ...
    <>
      <Container>
        <Row>
          {!isLoading && !isLoading2 ? (
            <>
              <h5 className="sandwich">{data[4]?.name}</h5>
              {data[4]?.products.map((dataGrilled, index) => (
                <Col
                  xs={6}
                  className="for-sandwich"
                  key={index}
