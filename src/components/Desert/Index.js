...
//           {!isLoading && !isLoading2 ? (
//             <>
//               <h5 className="sandwich">{data?.Desert?.name}</h5>
//               {data?.Desert?.products?.map((dataDesert, index) => (
//                 <Col
//                   xs={6}
//                   className="for-sandwich"
//                   key={index}
//                   onClick={() => handleShowModal(dataDesert)}
//                 >
...
//                         src={`${data2?.base_urls?.product_image_url}/${dataDesert?.image}`}
//                         // src={dataDesert.image}
//                         alt=""
//                       />
//                     </Col>
//                     <Col xs={7} className="sandwich-names-price">
//                       <div className="sandwich-both-name-discount">
//                         <h7>{dataDesert.name}</h7>
//                         <br />
//                         <h7>${dataDesert.price}</h7>
//                       </div>
//                     </Col>
...
const Index = () => {
  const { data, isLoading, isError, isFetching, isSuccess } =
    useGetAllPostQuery();
  // console.log("data for the Desert", data[7]);
  console.log("data for the Desert", data);
  const {
    data: data2,
    isLoading: isLoading2,
    isError: isError2,
    ...
    <>
      <Container>
        <Row>
          {!isLoading && !isLoading2 ? (
            <>
              <h5 className="sandwich">{data[7]?.name}</h5>
              {data[7]?.products?.map((dataDesert, index) => (
                <Col
                  xs={6}
                  className="for-sandwich"
                  key={index}
